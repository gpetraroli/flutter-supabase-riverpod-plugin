...
Widgets own layout and local form state; they never import `supabase_flutter` or call repositories. Notifiers own server-backed state and do not know table names or JSON keys. Repositories own queries and JSON ↔ model mapping. Services own cross-cutting I/O (for example storage).
...
- Add another feature as model → repository → notifier → screens
